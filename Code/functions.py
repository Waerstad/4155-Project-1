...
def Polynomial_Features(x, p, intercept=False):
    """
    Feature matrix for one variable.
    Take an array of x values, and the desired polynomial degree p.
    Create a feature (design) matrix with first column x**1, second column with x**2, and so on.
    Intercept=True, will turn the first column into ones, meaning the 2nd column will contain the value x**(1) and so on.
    """
    n = len(x)
    if intercept  == True:
        X = np.zeros((int(n), int(p + 1)))
        for i in range(0, int(p+1)):
            X[:, i] = x**i  # Create first column with only ones (since x**0 = 1), called the intercept or bias column
    else:
...
def OLS_parameters(X, y):
    ...
    Return the beta parameters of the OLS fit.
    """
...
def Find_MSE_Ridge_predict_poly_ridgelambda(x_data, y_data, poly_degree, ridge_lambda):
    """
    MSE for a spesifc configuration of poly_degree and ridge lambda.
    Take x data and y data, poly_degree as desired polynomial degree of model, and the parameter ridge_lambda.
    Return the MSE between between a test set of y and the model prediction trained on a train set of y. 
    """
    x = x_data
    y = y_data
